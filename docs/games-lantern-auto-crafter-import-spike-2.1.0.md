...
While the queue is staged, each queue card is an explicit planner selector. The selected card is highlighted, Brunt's native preview follows that weapon when available, and the queue-owned planner exposes its exact five-stat profile, perk targets, and blessing targets. Edits are session-local and reset the aggregate cost confirmation. Selecting or editing job 2 never advances, swaps, or otherwise mutates the melee-first execution cursor.
...
- Trait resolution compares Games Lantern's rendered perk ranges against Darktide's live localized trait descriptions, display-name keys, description keys, canonical IDs, and trait IDs. A reviewed semantic bridge translates Darktide's combat-system armor names (`super_armor`, `resistant`, `armored`, `berserker`) to player-facing Carapace, Unyielding, Flak, and Maniac names, but only for entries present in the selected weapon's live catalogue. Blessings additionally normalize the numeric `weapon_trait_<n>` identity from both the website image and Darktide texture path, but still require a unique name-compatible result inside that catalogue. Missing fields report the exact trait slot and queue job, target label, catalogue size, and bounded candidate IDs; no unavailable perk or blessing is substituted.

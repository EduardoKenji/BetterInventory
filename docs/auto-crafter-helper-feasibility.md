...
- Mastery blessing allocation follows vanilla `MasteryService.purchase_traits(pattern, operations)`, not the convenience `purchase_trait` wrapper: the latter catches a rejected backend PUT and resolves with the error, which previously made a failed request look submitted. The batch method exposes failed trait operations and performs vanilla sticker-book cache reset/warm behavior. After an accepted allocation, Phase 4 does not require immediate cache convergence. It polls a freshly reset sticker book with bounded exponential backoff (12 reads, roughly 48 seconds), proceeds only when the exact selected tier becomes `seen`, and distinguishes explicit backend rejection from bounded synchronization timeout. Each selected blessing is allocated from its lowest valid unseen tier through the desired tier before replacement; already-seen tiers and already-matching item traits are adopted. Perks remain Tier IV catalogue targets, require the mastery-to-20 workflow gate, and use the native `replace_perk_in_weapon(gear_id, one_based_slot, perk_id, nil, tier)` service signature. Blessings use the corresponding native `replace_trait_in_weapon` signature.
...
- **Implemented:** verify selected blessing tiers in the sticker book; purchase selected unseen tiers only when mastery allocation is enabled, then re-read and require `seen`.

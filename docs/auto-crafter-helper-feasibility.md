...
- Dump-stat shortcut buttons bind each native hotspot's `pressed_callback` inside the generated pass blueprint, matching Darktide's reliable button path. Late widget-content binding and per-frame `on_pressed` polling are prohibited here because presses can be lost when the scrollable panel is reflowed or the expanded workflow moves the grid within the scroll mask. Panel setting writes notify DMF normally so the controller refreshes its plan immediately and cannot show a stale dump-stat selection.
...
- Each run now freezes selected Brunt offer identity and price. Before every purchase POST, current native selection must still match frozen `offer_id`/`master_id`; changing weapon stops the run. Planner-setting changes also close dispatch gate instead of silently changing a live run.
...
- Default progress reporting is coalesced: first purchase plus every tenth purchase, and first sacrifice plus every fifth sacrifice. Polls and per-item preparation remain visible in panel state without creating notification spam.

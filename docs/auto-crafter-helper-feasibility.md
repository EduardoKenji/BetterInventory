...
- Position the panel against the right edge of Darktide's aspect-expanded virtual canvas, never at a fixed absolute X coordinate. Derive virtual width and height from `RESOLUTION_LOOKUP.width / render_scale` and `RESOLUTION_LOOKUP.height / render_scale`, preserve the validated 95-unit right and 110-unit top insets, clamp both axes to a 12-unit safe inset, and recompute when resolution or UI scale changes. This keeps the panel at X=1380 on 16:9 virtual 1920x1080, X=2040 on 3440x1440, and X=3300 on 5120x1440 instead of leaving it over Brunt's native catalogue.
...
- Phase 1C guarded purchase search requires an explicit per-run Craft click; the obsolete secondary mutation-gate setting has been retired. A user click starts one serialized purchase at a time through the native `StoreService.purchase_item(offer)` adapter. Every purchased item remains in inventory; misses are never silently discarded. Run start freezes the selected raw Brunt offer, summarized family, price, dump-stat ID, target, caps, and favorite preference. Closing Brunt detaches only the panel: purchase/mastery work continues while `Context.is_runtime_valid()` confirms the player remains in the Morningstar. Mission/loading/operative context exit, disabling Auto Crafter, configuration changes, explicit Stop, or failed reconciliation still stop future dispatch. Each purchase response supplies only the new `gear_id`; a fresh `fetch_gear` snapshot must contain that ID, expected family, and configured stat before the candidate can become best/exact. This prevents optimistic purchase payloads from falsely announcing a weapon absent from inventory. The loop stops before the next request when an enabled Ordo dockets cap, enabled maximum-purchases cap, wallet limit, or safety condition would be exceeded. At least one acquisition cap must be enabled before mutation search can start. Unknown item/stat shapes fail closed. Parallel mutation settings remain presentation-only; account mutations always stay serialized.

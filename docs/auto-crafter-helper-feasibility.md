...
- Dump stat is an enum control, not a one-direction mystery selector: its arrows wrap between `Damage` and `Auto-discover (future)`, and changing it updates the saved planner value in place.

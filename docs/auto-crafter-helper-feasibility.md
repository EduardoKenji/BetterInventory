...
Never assume every weapon family has `damage` as its dump stat. Auto mode must inspect the currently selected Brunt preview and choose the unique lowest valid canonical base-stat key. If preview data is unavailable, use matching weapon-family inventory only as a read-only fallback. If preview data is present but its minimum is tied, do not replace that authoritative ambiguity with an inventory guess: block acquisition and explain it. `damage` remains available as an explicit manual override, never as an implicit universal default.
...
- Dump stat is an enum control, not a one-direction mystery selector: its arrows wrap between manual `Damage` and live `Auto-discover`, and changing it updates the saved planner value in place. Auto mode displays the resolved stat name after the selected Brunt preview is available.

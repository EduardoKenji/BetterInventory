...
- **Dump stat**: a user-selected named base stat intended to be low, commonly exactly 60 in the proposed rule. Do not confuse this with overall base-stat rating. The selected weapon's discovered base-stat identities are the manual choices; Auto-discover is an additional recommendation option, never an implicit override.
...
Never assume every weapon family has `damage` as its dump stat. Candidate discovery and Auto recommendation are separate operations. The Brunt widget must expose every template-defined base-stat key for the currently selected weapon and retain Auto-discover as the sixth option when five stats are available, even when Brunt exposes no rolled values. Auto mode recommends the unique lowest valid canonical key only when authoritative rolled values exist. If the Brunt preview has no values, matching weapon-family inventory may provide a read-only fallback; sibling-mark values must be mapped by vanilla `display_name` back to the selected mark's exact canonical key. If no rolled values exist anywhere, keep all manual choices available and report `Auto recommendation unavailable (choose a stat)` instead of guessing or waiting indefinitely. If preview data is present but its minimum is tied, do not replace that authoritative ambiguity with an inventory guess: block acquisition and explain it. Explicit manual choices remain authoritative; legacy/common aliases such as `damage`, `mobility`, `penetration`, `first_target`, and `defenses` may resolve to the selected weapon's exact raw stat key, but an unavailable explicit choice must block rather than silently switch to Auto.
...
- Weapon selection changes reconcile the saved manual dump stat against the new template catalogue. A semantically compatible stat is remapped by vanilla display key to the new weapon's exact canonical key; an incompatible stat selects the first stable, localized manual option in the new catalogue. Do not show or preserve a stale choice from the previous weapon, and do not park an automatic transition on an unavailable Auto recommendation. While a new catalogue is genuinely empty or loading, defer reconciliation instead of changing prematurely. Auto may still report unavailable when the user explicitly selects it and neither a rolled preview nor compatible inventory evidence exists; the weapon template contains candidate names but no authoritative best-dump-stat recommendation.
...
- Dump stat is a contextual enum control, not a one-direction mystery selector: its arrows wrap through the selected weapon template's complete localized base-stat catalogue and then Auto-discover as the final option. Changing it updates the saved planner value in place. Auto mode displays the resolved recommendation only when authoritative rolled values are available; otherwise it explicitly asks the user to choose one of the still-available manual stats. Manual choices display vanilla labels such as `Finesse`, never raw implementation IDs, and are never replaced by Auto.
...
- Phase 1B read-only planner is now implemented. It exposes the selected weapon template's complete base-stat catalogue, explicit manual dump-stat selection, Auto-discover as the final recommendation option, optional Ordo dockets and maximum-purchase caps, best-candidate fallback, request scheduling mode, and selected-weapon perk/blessing catalogue discovery. Candidate discovery uses `WeaponTemplate.weapon_template_from_item(master_item).base_stats`, so weapons such as Crowbar remain selectable when Brunt exposes only `lootChoices`, no store-item preview can be instantiated, and no matching inventory item exists. Rolled preview or bounded matching-inventory values are used only to calculate Auto's unique-lowest recommendation. Cross-mark fallback maps by vanilla display key back to the selected mark's exact raw key. When no values exist, manual choices remain available and Auto reports that its recommendation is unavailable. UI labels use vanilla localization (`Finesse`, `Penetration`, and so on); canonical raw IDs remain internal. Ambiguous or malformed data blocks mutation rather than guessing. Selected-weapon trait discovery performs bounded sequential reads through native crafting metadata, mastery-family, and trait-sticker-book services, returning canonical perk IDs/tiers and valid blessing IDs/tier statuses. Discovery is cached per selected offer and cancelled on target/context change. The widget builds a fail-closed preflight from the currently selected native Brunt offer and reports a conservative docket floor/cap; plasteel and diamantine remain explicitly deferred until a candidate exists. The widget's `Craft (read-only preview)` action only rebuilds this plan and emits a notification; it does not call purchase, crafting, sacrifice, mastery, favorite, or rename operations.
- Phase 1C guarded purchase search is now implemented behind `auto_crafter_allow_mutations`, which defaults off. A user click starts one serialized purchase at a time through the native `StoreService.purchase_item(offer)` adapter. Every purchased item remains in inventory; misses are never silently discarded. The loop parses authoritative purchased gear, stops on the configured exact dump-stat target, and stops before the next request when an enabled Ordo dockets cap, enabled maximum-purchases cap, wallet limit, or safety condition would be exceeded. At least one acquisition cap must be enabled before mutation search can start. Unknown item/stat shapes fail closed. Parallel mutation settings remain presentation-only; account mutations always stay serialized.
...
- Phase 3 mastery-to-20 repetition is now implemented as an opt-in serialized state machine. When `auto_crafter_level_mastery_20` is enabled, the exact dump-stat purchase remains protected as the final candidate; each imperfect purchase is re-read against live mastery state and becomes one Redeemed/sacrificed fodder item only while mastery is below 20. The loop never overlaps purchase, upgrade, extraction, claim, refresh, or mastery-read requests. It requires authoritative item-family/gear/dump-stat reconciliation before final completion, polls mastery after every sacrifice, preserves the target when a cap is reached, and stops fail-closed on context exit, setting changes, missing IDs, stale gear, failed refresh, zero-XP extraction, or bounded sync timeout.
...
- The complete planned configuration surface is now implemented in both DMF settings and the Brunt panel: purchase search, mastery-to-20, mastery-point allocation, Transcendent consecration, expertise 500, perk/blessing replacement and two target slots each, favorite, rename, and request mode. Dependency-disabled children preserve their saved values and become interactive immediately when prerequisites are enabled. The two acquisition-cap toggles gate the guarded Phase 1C/3 purchase search; selected-weapon perk/blessing catalogue discovery remains read-only, Phase 3 consumes only the mastery-to-20 toggle, and target replacement, output handling, and request-mode mutation behavior stay backend-inert until their guarded implementation phases land. Static verification rejects accidental wiring of the still-inert options.
...
- Phase 3 still requires live validation with a disposable test weapon before broader use, especially gear/stat representation, cost responses, mastery claim lag, context cancellation, and interactions with Quick Level Mastery or other mutation mods. The implementation intentionally keeps parallel mutation mode inert and does not attempt final consecration, expertise, perk/blessing replacement, favorite, or rename work.

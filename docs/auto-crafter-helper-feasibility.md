...
- Each run records a monotonic start timestamp plus update-time fallback. The completion report stores total elapsed seconds, the HUD briefly shows a human-readable `Crafting complete in ...` result, and the final native notification includes the same duration.

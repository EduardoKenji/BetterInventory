...
- Dump-stat shortcut buttons bind through each native hotspot's `pressed_callback`, matching arrow and checkbox input routing. Per-frame `on_pressed` polling is prohibited here because presses can be lost when the scrollable panel is reflowed or the expanded workflow moves the grid within the scroll mask.

...
| Switch an explicitly selected weapon mark | `Managers.data_service.mastery:switch_mark(gear_id, mark_id)` | `PATCH /data/<account>/account/gear/<gear_id>`, body `{ mdi=mark_id }` | Replaces master-data identity on the same gear near the end of Phase 4; completion requires a fresh exact-mark inventory match. |
...
- `.../scripts/backend/mastery.lua:26-39` defines mark switching as a gear `mdi` PATCH shared by melee and ranged weapons.
- `.../scripts/managers/data_service/services/mastery_service.lua:296-301` catches mark-switch rejection and resolves its error value; Promise resolution is therefore not proof of mutation success.
- `.../scripts/utilities/mastery.lua:711-750` enumerates milestone marks and sets `unlocked` from `claimed_level + 1`.
- `.../scripts/ui/views/inventory_weapon_marks_view/inventory_weapon_marks_view.lua:520-539` reads marks by `parent_pattern` and permits selection only when milestone data says unlocked.
- `.../scripts/ui/views/inventory_background_view/inventory_background_view.lua:151-210` dispatches native switch, invalidates gear cache, refetches inventory, and repairs equipped/loadout references.
...
- Cross-mark acquisition never copies a sibling mark's raw key into the frozen purchase plan. The planner freezes the selected mark's canonical key plus Darktide display-name key; candidate policy may read a random Brunt sibling only through one exact, unique display-key match. Missing or duplicate matches remain unavailable. This identity bridge applies consistently to single dump-stat search, inventory reuse, Phase 3, and Phase 4 while final selected-mark verification still requires the exact `master_id`.
...
| Choose blessing targets | selectors | level-to-20 and change-blessings checked | Values discovered from trait category/sticker book. Allocation may stay disabled when selected tiers are already owned; fresh sticker-book verification fails closed if they are not. |
...
Default scheduling must not run mutations against the same ordered resource concurrently. Brunt purchases remain one-at-a-time because they share a wallet transaction ID. Phase 3 has one explicit allowlisted exception: one ordered fodder rarity upgrade may overlap the next ordered Credits purchase while the run owns the mutation arbiter. Rarity upgrades never overlap each other because crafting requests share material and cache state. Extraction waits for the upgrade lane and one authoritative gear refresh. No other mutation pair is parallelized. During reward-sync backoff, default mode may interleave one independent final-item mutation before the next read. Empowering beyond current claimed cap must wait.
...
- Keep section headers full-width with a right chevron and optional count. Planner, exact weapon Marks, trait targets, workflow, resuming options, and Estimates collapse independently. Marks indexes all local master items sharing selected Brunt family's authoritative `parent_pattern`, weapon slot, and weapon-template contract; `lootChoices` is fallback only because it may be filtered by current mastery unlocks. Thus every mark remains selectable independent of mastery level without duplicating native Melee Weapons and Ranged Weapons lists. Estimates owns search budget and all projected currency rows and sits immediately above Craft.
...
- Phase 3 mastery-to-20 uses projected XP as a conservative stop-spending gate. The exact dump-stat target is protected; run-owned misses are queued, and only the minimum prefix estimated to reach level 20 is extracted. Credits purchases remain ordered. One ordered rarity-upgrade lane overlaps the next purchase; each upgrade has generation guards and a 45-second timeout. Rarity upgrades never overlap each other. Extraction cannot start until the lane settles and a single authoritative gear refresh confirms identity, family and Redeemed rarity. One multi-item extraction advances local mastery immediately; milestone claiming and bounded authoritative convergence remain the final barrier. Every surviving run-owned spare is discarded after protected reconciliation.
...
- Manual Brunt planning can optionally replace the one-dump-stat criterion with an exact five-stat allocation. The panel derives all five contextual names from the selected weapon catalogue, exposes two cells per row plus a total cell, adjusts in whole points from 60 through 80, and prevents UI increments above an aggregate 380. Craft requires exactly 380 and reports the current invalid total before account-operation ownership or purchase dispatch. The frozen allocation is checked against projected level-500 stats for purchase acceptance, family-equivalent inventory reuse (including sibling marks whose raw stat IDs differ but resolve to the same unambiguous display identity), Phase 3 reconciliation, and final Phase 4 verification. Modern Games Lantern cards preserve a uniquely resolved five-stat 380 profile by stat identity; staged queue-card selection exposes session-local editing without changing the melee-then-ranged execution cursor.
...
- Weapon-mark selection freezes an exact desired `master_id` only when the user explicitly chooses a Marks entry. Brunt still purchases at family level, so the acquired or reused weapon may initially be a sibling mark. Phase 4 performs the native `MasteryService.switch_mark` operation after rarity, expertise, mastery, perk, and blessing work, then invalidates Darktide's gear cache and requires a fresh inventory item with the same `gear_id`, family, and selected `master_id`. The native service wrapper catches backend rejection and resolves the error value, so Promise resolution is never treated as success; an unchanged mark fails the run instead of producing a false completion. Runs without an explicit Marks choice retain Brunt's acquired mark and dispatch no mark mutation. External native mark changes remain covered by the shared account-mutation guard while Auto Crafter owns a run.
- Mark discovery is mastery-level-independent so every locally installed sibling mark can be planned. Discovery accepts only resolved master items with a non-empty ID, the same `parent_pattern`, the same melee/ranged slot (`slot_primary` or `slot_secondary`), matching weapon category, and a concrete weapon template. Duplicate, unresolved, cross-family, cross-slot, and missing-template candidates are omitted. Invalid/stale manual mark selection is a no-op and emits `mark_selection_rejected` diagnostics. Inventory resume and final reconciliation use mastery-family identity because marks are interchangeable without crafting cost.
...
- Widget now has a guarded `> CLICK HERE TO STOP / INTERRUPT <` action in every active phase. It closes future dispatch immediately but does not pretend an already submitted account mutation can be cancelled; that request settles behind a generation guard and its continuation remains inert.
...
Quick Level Mastery demonstrates the throughput benefit of overlapping independent requests, but its click path launches purchases and upgrades without a synchronization barrier and defers extraction until view exit. Auto Crafter uses a bounded version: wallet purchases remain serial, one ordered rarity upgrade may overlap the next purchase, and extraction waits for the upgrade lane plus one gear read. Confirmed extraction removes consumed IDs locally and advances projected XP without per-item mastery polling. Once projection reaches 20, the controller claims milestones once and polls authoritative XP/claim convergence before dependent crafting. Live failure traces showed concurrent rarity upgrades can reject with `unknown backend error`, so same-resource crafting overlap is deliberately forbidden.

...
`Allocate mastery points` needs an explicit deterministic policy. Default: purchase selected blessing tiers whenever their next valid tier is unlocked. Darktide also gates Tier II-IV blessing purchases by total mastery points already spent across the tree, so the allocator must use the live `tierCosts` and `tierThresholds` response and buy deterministic lowest-order prerequisite tiers across compatible blessings whenever a selected tier is still locked. It then returns to the selected targets and stops once both requested tiers are owned, leaving any unnecessary points unspent. Never distribute points randomly. If no legal prerequisite exists or selected targets cannot be funded, halt before replacing anything.
...
Generated base item level range: 290-330
Profane -> Transcendent: [dockets] - [plasteel] range [diamantine] range
Base item level -> 500: [dockets] - [plasteel] range [diamantine] range
Mastery 20 fodder: 3-4 Redeemed weapons / 17,900 XP remaining
...
- Keep section headers full-width with a right chevron and optional count. Search limits, workflow, trait targets, final handling, advanced safety, Melee Weapons, and Ranged Weapons collapse independently; both weapon sections and the less-frequent target/output/advanced sections start collapsed.
...
- Perk and blessing targets are contextual enum steppers populated from the selected weapon's live catalog. Options begin with `Keep current` and `Auto-select`, followed by every compatible localized canonical target. Perk labels must use the exact vanilla Refine Item path, `Items.trait_description(perk_item, tier, 1)`, not `perk_item.display_name`; the latter is often an internal content path such as `Content/Items/Perks/...`. Persist perk ID and tier as one distinct option identity so equal IDs at different tiers cannot collapse into one selector value. Changing weapon invalidates stale targets back to `Keep current`. Raw trait IDs never appear unless vanilla formatting genuinely fails.
...
- Phase 1C guarded purchase search is now implemented behind `auto_crafter_allow_mutations`, which defaults off. A user click starts one serialized purchase at a time through the native `StoreService.purchase_item(offer)` adapter. Every purchased item remains in inventory; misses are never silently discarded. Run start freezes the selected raw Brunt offer, summarized family, price, dump-stat ID, target, caps, and favorite preference. Closing Brunt detaches only the panel: purchase/mastery work continues while `Context.is_runtime_valid()` confirms the player remains in the Morningstar. Mission/loading/operative context exit, disabling mutations, configuration changes, explicit Stop, or failed reconciliation still stop future dispatch. Each purchase response supplies only the new `gear_id`; a fresh `fetch_gear` snapshot must contain that ID, expected family, and configured stat before the candidate can become best/exact. This prevents optimistic purchase payloads from falsely announcing a weapon absent from inventory. The loop stops before the next request when an enabled Ordo dockets cap, enabled maximum-purchases cap, wallet limit, or safety condition would be exceeded. At least one acquisition cap must be enabled before mutation search can start. Unknown item/stat shapes fail closed. Parallel mutation settings remain presentation-only; account mutations always stay serialized.
- Phase 2 one-item mastery primitive is now implemented behind the same gate. It accepts one explicit purchased candidate, verifies it still exists and belongs to the expected weapon family, reads authoritative mastery before any upgrade, and skips all mutation if mastery is already 20. When a rarity upgrade is required, it refreshes mastery again after the confirmed upgrade and immediately before destructive extraction, trading one read for a fresher claim baseline. It upgrades to Redeemed only when needed, sacrifices exactly one `gear_id`, requires positive XP plus confirmation of that ID, verifies the item disappeared from a fresh authoritative gear read, claims tiers from the pre-sacrifice mastery object plus returned XP exactly once, then polls fresh mastery/claim state with bounded attempts. Level 20 counts as XP convergence even when backend XP is capped below the arithmetic `before + award` value. The primitive is internal to Phase 3; the unsafe standalone `Redeem + sacrifice one` widget action was removed so a completed target cannot be destroyed by an ambiguous click.
- Phase 3 mastery-to-20 repetition is now implemented as an opt-in serialized state machine. When `auto_crafter_level_mastery_20` is enabled, the exact dump-stat purchase remains protected as the final candidate; each imperfect purchase is re-read against live mastery state and becomes one Redeemed/sacrificed fodder item only while mastery is below 20. With best-candidate fallback enabled, one live best miss is reserved; a better miss replaces it and the prior reserve becomes fodder. This ensures a cap stop returns an item that still exists rather than a previously sacrificed `gear_id`. The loop never overlaps purchase, upgrade, extraction, claim, refresh, or mastery-read requests. It requires authoritative item-family/gear/dump-stat reconciliation before final completion, polls mastery after every sacrifice, preserves the target when a cap is reached, and stops fail-closed on Morningstar context exit, setting changes, missing IDs, stale gear, failed refresh, zero-XP extraction, or bounded sync timeout. Brunt selection changes after Start do not retarget the frozen background run.
...
- Trait target UI now offers optional synchronized grids beneath the existing arrow selectors. Perks use four columns and BetterInventory's injected heavy-compression adapter; blessings use three columns and native `Items.trait_textures` icons rendered through Darktide's `traits_container` material with icon/frame shader values. Both grids grow to the discovered catalogue size, include `Keep current` at semantic index 0 and `Auto-select` at index 1, and default target 1/target 2 to those two distinct positions. Left click assigns target 1 with a yellow highlight; right click assigns target 2 with a green highlight. Right-click routing has priority because Darktide may expose both `on_pressed` and `on_right_pressed` on the same RMB frame. Grid clicks and arrows update the same persisted targets, and pair reconciliation prevents duplicate selections by moving the displaced target to the first valid distinct option. Grid visibility defaults off, obeys the same mastery/workflow prerequisites as its target selectors, and preserves the panel's absolute scroll position across reflow. Icon and compact-label formatting enter the standalone panel through optional host adapters/data fields, preserving later extraction from BetterInventory.
...
- The complete planned configuration surface is implemented in both DMF settings and the Brunt panel: purchase search, inventory-base reuse, mastery-to-20, mastery-point allocation, Transcendent consecration, weapon level 500 (backend operation remains `addExpertise`), perk/blessing replacement and two target slots each, favorite, rename, persistent status HUD, and request mode. Dependency-disabled children preserve their saved values and become interactive immediately when prerequisites are enabled. Default-on checkboxes preserve an explicit saved `false`; settings adapters apply defaults only to `nil` or failed reads. Exact authoritative results use guarded `Items.set_item_id_as_favorite` plus immediate `Items.is_item_id_favorited` verification when favorite is enabled. Rename and parallel-mutation behavior remain backend-inert; Phase 4 final crafting is now connected and always serialized.
- Phase 4 final crafting is implemented behind the existing account-mutation gate and explicit Craft click. After Phase 3 (or immediately when mastery leveling is disabled), it freezes the final `gear_id`, family, projected level-500 dump stat, discovered trait category, selected Tier IV perks, selected blessing tiers, and enabled final operations. It consecrates one rarity step at a time to Transcendent, advances expertise at displayed 100-level milestones to 500, allocates only selected unseen blessing tiers when authorized, and replaces perk/blessing slots one at a time. Every mutation is followed by a fresh authoritative gear or sticker-book read; missing identity, wrong family, projected-stat drift, unconfirmed rarity/level, unseen blessing, or unconfirmed replacement stops future dispatch. Compatible manual progress is adopted because each step recomputes from current state. Weapon-level notifications are emitted only for authoritative levels divisible by 100 (normally 400 and 500 from a 290-330 Brunt weapon).
- Inventory-base reuse is enabled by default. Before spending at Brunt, the controller searches the authoritative gear snapshot for the same weapon family and exact projected dump-stat target. Non-favorite matches are eligible by default; an explicit disabled-by-default option permits favorites. A reused item enters the same favorite/mastery/Phase-4 pipeline and produces no purchase spend. It is never unfavorited and cannot bypass final identity/stat verification.
...
- Workflow scope copy distinguishes connected purchase/mastery behavior from saved-only future options. Parallel-read mode copy now states actual behavior: preference is saved, while current reads and all mutations remain serialized.
...
- **Implemented:** verify selected blessing tiers in the sticker book; purchase selected unseen tiers when mastery allocation is enabled, automatically satisfy live mastery-tree spend thresholds through deterministic prerequisite allocations, then re-read and require every submitted tier to become `seen`.

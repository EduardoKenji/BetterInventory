...
- Keep section headers full-width with a right chevron and optional count. Planner, exact weapon Marks, trait targets, workflow, resuming options, and Estimates collapse independently. Marks list only selected Brunt family's authoritative `lootChoices`; it never duplicates native Melee Weapons and Ranged Weapons lists. Estimates owns search budget and all projected currency rows and sits immediately above Craft.

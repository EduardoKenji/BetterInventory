...
```text
Acquisition: 1-43 purchases / 11,600-500,000 dockets
Upgrade estimate range: starting base item level 290-330
Configured total: [dockets icon] 11,600-500,000 [plasteel icon] 12,400-15,800 [diamantine icon] 2,100-2,700
```

Do not display an average perfect-roll probability or average acquisition cost. Brunt exposes results but no authoritative roll-weight distribution, and a uniform-composition calculation would look exact while modeling the wrong process. Acquisition forecasting is therefore bounded by current offer price plus enabled Docket/purchase caps. Material forecasting reads live backend crafting recipes and reports a range for a candidate starting at base item level 290-330. Recompute from the authoritative item immediately after purchase; that exact quote replaces the range before any mutation.

Material range includes only connected/configured operations whose live recipe can be evaluated. Show an em dash for unknown replacement, blessing-unlock, or mastery-fodder costs instead of folding invented values into the total. Currency values use small vanilla Docket, Plasteel, and Diamantine icons; text remains present for accessibility and exact values.
...
- Perk and blessing targets are contextual enum steppers populated from the selected weapon's live catalog. Options begin with `Keep current` and `Auto-select`, followed by every compatible localized canonical target (perk tier shown when relevant). Changing weapon invalidates stale targets back to `Keep current`. Raw trait IDs never appear unless localization is genuinely unavailable.

...
- Dump stat is a contextual indexed enum control: its arrows wrap through the selected weapon template's complete localized base-stat catalogue. Common stats use the fixed semantic priority Damage, Mobility, Finesse, First Target, Penetration; remaining weapon-specific stats follow deterministically. Changing it updates the saved planner value in place. Changing weapons resets to visible index 0. Labels use vanilla text such as `Finesse`, never raw implementation IDs.

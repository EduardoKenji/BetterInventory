...
- Inventory-base reuse is enabled by default. Before spending at Brunt, the controller searches the authoritative gear snapshot for the same weapon family and exact projected dump-stat target. Favorite state must be readable. Non-favorite matches are eligible by default; an explicit disabled-by-default option permits favorites. Selection is deterministic: highest current expertise, then rarity, then stable gear ID. A reused item enters the same favorite/mastery/Phase-4 pipeline and produces no purchase spend. It is never unfavorited and cannot bypass final identity/stat verification.

...
- Dump stat is a contextual enum control, not a one-direction mystery selector: its arrows wrap through the selected weapon's discovered base-stat keys and then Auto-discover as the final recommendation option. Changing it updates the saved planner value in place. Auto mode displays the resolved recommendation after the selected Brunt preview is available; manual choices display their exact resolved key and are never replaced by that recommendation.
...
- Phase 1B read-only planner is now implemented. It exposes the selected weapon's live base-stat candidates, explicit manual dump-stat selection, Auto-discover as the final recommendation option, optional Ordo dockets and maximum-purchase caps, best-candidate fallback, request scheduling mode, and selected-weapon perk/blessing catalogue discovery. Brunt's `lootChoices` descriptions do not always expose a usable rolled preview through `MasterItems.get_store_item_instance(description)`, so discovery now tries the native preview, direct master-item stats, and a bounded inventory fallback. The inventory fallback scans up to 1,024 gear entries and matches parent pattern, master ID, or weapon-card display name; it then exposes the exact candidate keys and recommends the unique lowest valid canonical base-stat key. Ambiguous or missing data blocks mutation rather than guessing. Selected-weapon trait discovery performs bounded sequential reads through native crafting metadata, mastery-family, and trait-sticker-book services, returning canonical perk IDs/tiers and valid blessing IDs/tier statuses. Discovery is cached per selected offer and cancelled on target/context change. The widget builds a fail-closed preflight from the currently selected native Brunt offer and reports a conservative docket floor/cap; plasteel and diamantine remain explicitly deferred until a candidate exists. The widget's `Craft (read-only preview)` action only rebuilds this plan and emits a notification; it does not call purchase, crafting, sacrifice, mastery, favorite, or rename operations.

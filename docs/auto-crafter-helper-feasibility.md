...
- The complete planned configuration surface is now implemented in both DMF settings and the Brunt panel: purchase search, mastery-to-20, mastery-point allocation, Transcendent consecration, weapon level 500 (backend operation remains `addExpertise`), perk/blessing replacement and two target slots each, favorite, rename, and request mode. Dependency-disabled children preserve their saved values and become interactive immediately when prerequisites are enabled. Default-on checkboxes must preserve an explicit saved `false`; settings adapters may only apply defaults to `nil` or failed reads. The two acquisition-cap toggles gate the guarded Phase 1C/3 purchase search; selected-weapon perk/blessing catalogue discovery remains read-only, and Phase 3 consumes only the mastery-to-20 toggle. Exact authoritative results now use guarded `Items.set_item_id_as_favorite` plus immediate `Items.is_item_id_favorited` verification when favorite is enabled. Rename, target replacement, mastery allocation, final consecration/level work, and request-mode mutation behavior stay backend-inert until their guarded implementation phases land.

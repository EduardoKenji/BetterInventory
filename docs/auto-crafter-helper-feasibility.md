...
| Switch existing weapon mark (native/manual only) | `Managers.data_service.mastery:switch_mark(gear_id, mark_id)` | `PATCH /data/<account>/account/gear/<gear_id>`, body `{ mdi=mark_id }` | Replaces master-data identity on same gear. Auto Crafter intentionally never dispatches this operation. |
...
- Weapon-mark selection is planning identity, not a gear mutation. Brunt supplies the selected exact master item, and inventory reuse requires that exact `master_id` whenever both sides expose it. Auto Crafter never calls `MasteryService.switch_mark`, before or after mastery 20. Phase 3 must converge authoritative level 20 plus claimed tier 19 before Phase 4 when mastery leveling is enabled; when it is disabled, exact-mark acquisition still makes mark switching unnecessary. Darktide's native mark operation is a `PATCH /account/gear/<gear_id>` with `{mdi=<mark_id>}`. Its service wrapper catches backend rejection and resolves the error value, so automation must not infer success from Promise resolution. External native mark changes are covered by the shared account-mutation guard while Auto Crafter owns a run.

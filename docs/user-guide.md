...
When other mods add more weapon actions than fit beside the weapon details, BetterInventory caps the native Marks/Cosmetics/Inspect panel at **Weapon-action panel maximum height** and lets Darktide's own masked grid scroll the remaining actions. The Debug group can expand the panel to 5, 10, or 20 total rows with inert test buttons for presentation and controller-navigation checks; reopen the inventory after changing either option.

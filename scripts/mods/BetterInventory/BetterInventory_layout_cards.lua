...
local function compound_weapon_static_icon(item)
	...
	-- Build a dense list. Lua's ipairs stops at the first nil, while backend
	-- items do not consistently expose weapon_template, name, and master-data
	-- identity together.
...
local function clear_static_compound_icon(widget)
	...
	if not content or content.better_inventory_static_compound_weapon_icon ~= true then
		return false
	end
	...
	content.better_inventory_static_compound_weapon_icon = nil
	content.use_placeholder_texture = 1

	return true
end

local function apply_static_compound_icon(widget, element)
	local item = item_from_element(element or widget and widget.content and widget.content.element)
	local icon = compound_weapon_static_icon(item)
	local content = widget and widget.content
	local icon_style = widget and widget.style and widget.style.icon
	local material_values = icon_style and icon_style.material_values

	if not icon or type(content) ~= "table" or type(material_values) ~= "table" then
		return false
	end
	...
	content.better_inventory_static_compound_weapon_icon = true

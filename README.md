...
- Standalone **Custom Item Names and Colors**: select an inventory item and press the configurable editor key (E by default) to change its name, name color, or card-background color, or restore its defaults. Changes are stored per gear ID, survive restarts, refresh the selected card immediately, and are removed when Darktide deletes the item. Reset confirmation prompts are skipped by default. Name It remains optional: its names take priority while **Enable 'Name It' override** is on, while disabling that option exposes BetterInventory names such as `Dueling Sword Mk IV` or names saved by this editor. The same section owns the default-on Curio title option, which reserves a responsive two-line name area above all four attributes with or without Name It across Character Overview, Inventory, Hadron, the Armoury Exchange, GlobalStore and other supported grids.

...
- Visible Equipment compatibility: a default-on **Mod Integration: Visible Equipment** option preserves that mod's injected Primary and Secondary Placement `gear_placement_slot` widgets in the Cosmetics tab. BetterInventory continues rendering its configured detailed melee, ranged and Curio cards in the Loadout tab, so both mods work simultaneously. Disabling compatibility restores BetterInventory's original broad slot detection and is intended only for troubleshooting.

...
- Independent 9–20 px Curio font controls: the primary innate line defaults to 14 px while the three secondary lines default to 13 px. The secondary control is greyed out in the Primary stat profile, and detailed-card row positions and automatic height follow the selected values.
- Configurable 0–20 px vertical grouping between the primary Curio stat and its three secondary lines, defaulting to a subtle 3 px Inventory2D-style gap. The control applies only to the All four stats profile and contributes to automatic card height.
...
- Compact Armoury store cards preserve both item power and purchase state. A dedicated 34 px footer keeps wallet/price or owned text in the lower-left and item power in the lower-right. Blessings are anchored above that footer, and at the default Curio typography automatic card height grows to 114 px for those rows, 129 px for a detailed four-line Curio, or 148 px when two weapon-perk rows are also enabled.

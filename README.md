...
- Default-on weapon blessing symbols using Darktide's current composited blessing material; the native ranked frame visibly carries each blessing level. Icon size is configurable from 20–48 px (36 px by default), with a configurable 0–20 px horizontal gap defaulting to 3 px.
...
- Weapon perk rows are forced to one line, shrink to an 8 px minimum and then use an ellipsis. Their native rank symbols have an independent 12–32 px size control (17 px by default). Automatic height uses both configured icon sizes when reserving the perk, blessing and store-footer regions, instead of allowing them to collide with one another or item power.
- An opt-in precise weapon-attribute display replaces only each native bar's rounded numerator with its stored one-decimal value, such as `[79.6/80]%`. Bar geometry, maximum values, gameplay calculations and active expertise previews remain native.
...
The current implementation does not alter filters or backend transactions. Its only sorting change is the optional inventory priority wrapper described above; the native comparators remain authoritative within each priority group. It has been statically checked against Darktide 1.12.3 source, but the new Hadron and Armoury paths plus the on-page inventory checkboxes still require mouse/controller, two/three/global-four-or-five-column, purchase-state and hot-enable/disable passes in game.
...
- Weapon base-stat entries are different: `WeaponStats.get_comparing_stats` exposes the unrounded `item.base_stats[].value` fraction, while Darktide's normal blueprint rounds its display through `Items.preview_stats_change`. BetterInventory can therefore show a truthful one-decimal numerator without changing the underlying item or bar.
...
The next step is an in-game smoke-test matrix covering the existing character inventories plus Entreat Hadron and Requisition Weapons & Curios. Verify that the Curio priority checkbox follows the bottom of details, that melee/ranged checkboxes sit below Marks/Cosmetics/Inspect, and that all three controls mirror the same persistent mod option. Under every native sort, confirm equipped then favorite items remain first and that equipping or changing a favorite reorders immediately. On a weapon whose stored stat is fractional, confirm that precision changes only the normal attribute numerator and that an expertise preview remains native. For both vendor views, test two and three global columns, verify that global four/five still renders exactly three, exercise mouse/controller selection and scrolling, and check active/owned/unaffordable Armoury cards. Confirm that Sacrifice Weapons, Brunt's Armoury and Multi-Operative Supply remain native. Hot enable/disable and coexistence with GlobalStore and active sorting/information mods also need validation.

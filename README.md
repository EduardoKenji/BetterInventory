...
- Two to five responsive columns calculated from the current grid width, with independent **Melee Weapons Columns**, **Ranged Weapons Columns** and **Curios Columns** controls (all default to three). Existing profiles migrate the retired global `columns` value to all three controls once; afterward each slider is independent. Armoury Exchange and GlobalStore honor the matching category value but clamp it to three columns.
- A dedicated Armoury weapon-name font-size control (14 px by default) so long Requisition Weapons & Curios names can be kept compact without changing inventory or Hadron cards.
- A grid-layout switch: disabling it preserves Darktide's native single-column width, layout behavior and icon lifecycle while retaining enabled BetterInventory content enhancements. Single-column cards reserve additional height for BetterInventory's perk and blessing rows and can show both full framed blessing icons beside their compact text lines. Default-on standalone weapon-modifier data shows all five maximum-potential stats in single-column mode and the lowest maximum-potential stat on grid cards; Quick Look Card is no longer required, while its passes are safely reused or suppressed when installed to prevent duplicates. These controls retain the established `Mod Integration: Quick Look Card` section name. The lowest-stat highlight keeps the existing pink default but exposes preset, RGB and opacity controls. The grid label can appear above weapon power or beside the weapon name, and the single-column block has independent font-size, label/value gap, and horizontal/vertical position controls.
- Automatic inventory-window expansion when the requested per-slot columns would otherwise force cards outside the visible content area; this can be disabled to shrink cards instead. Melee/ranged inventories have an additional 0–120 px width slider, defaulting to 80 px, plus a threshold dropdown that applies it independently to whichever weapon inventory reaches four or five columns. The default adds 20 px per card across four columns or 16 px across five. Requested expansion is canvas-clamped with a 16 px safety margin so extreme spacing/width combinations narrow their cards instead of pushing details or actions off-screen.
...
- A final **Equipped item highlight** subsection at the bottom of **Card content**, with a selector offering **Off**, **Soft glow**, **Animated dashed border**, **Pulsing animated dashed border**, and **Solid border**. The default is a gold Animated dashed border at width 2. Soft glow has an independent 0-100% intensity slider; both dashed modes share an independent 1-5 width slider and Solid border has its own. Soft glow's mode default is white, while borders use the Auto Crafter terminal gold. Shared presets and RGB sliders apply to every mode, and editing a channel selects Custom colour. The dashed modes use Darktide's native GPU-animated frame material. Pulsing dashes additionally fade from 15% to 100% opacity and back over a four-second cycle using Darktide's global UI clock, with no retained per-card timer state or per-frame allocation. Bounded native passes are replaced during blueprint recomposition. Every mode follows Darktide's native equipped state without hiding its equipped symbol.
- A separate final **Newly acquired item highlight** subsection that replaces Darktide's small new-item dot with a green Pulsing animated dashed border at width 2 by default. Native dot only, Soft glow, Animated dashed border, and Solid border modes are also available, with the same bounded width/intensity and preset/RGB controls. BetterInventory follows Darktide's authoritative persisted new-item marker rather than maintaining acquisition history. Items are acknowledged through Darktide's native removal callback when selected by default; an optional Hover mode acknowledges them as soon as a mouse pointer enters the card, while controller focus also counts as acknowledgement. Pulsing and acknowledgement share the first pass safely, callback failures are contained, and enhanced layers are bounded and replaced during blueprint recomposition.
...
- Curio rarity colour strips are synchronized on initial creation and equipped-item replacement across supported hybrid/native cards. The default-off **Use native Curio overlay** option restores Darktide's ornate inner frame and portrait geometry for populated detailed Character Overview Curios while retaining BetterInventory's title, stat lines and item level.
...
- An invariant scalable inventory-options panel. The default-on **Show inventory options widget** setting is its sole visibility control. After reopening an inventory, the panel places synchronized Sorting and discard controls inside one Darktide-managed bounded panel with masked scrolling and clickable collapsible section headers. It exposes width, maximum height, row spacing and independent top/bottom/left/right padding controls; its defaults are 445 x 360 px, 8 px row spacing, 10 px vertical padding and 12 px horizontal padding. Additional rows scroll instead of extending into the footer. Curio views additionally expose full information-panel width and upper preview-area height percentages. Preview art scales on both axes with the upper area, preserving its aspect ratio. Native discard mode presents Sorting only. The established loose-widget implementation remains only as an automatic initialization-failure fallback.
...
- An **Automatic Curio Buyer**, disabled by default, with synchronized controls in mod options and the scalable inventory panel. It scans the normal Armoury Exchange across all saved characters and can filter by displayed item level, Health, Toughness, Stamina, Wound, class or individual character. Morningstar entry remains supported; an additional Operative Selection scan is opt-in and default-off. The default account-scoped rotation gate allows at most one pass per Armoury refresh across both contexts, using backend store-boundary metadata rather than a rolling timer. Rotation-triggered passes require the returned storefront to advertise a boundary beyond the prior rotation; an expired response is retried and cannot consume the new rotation before Darktide publishes it. The 2.0.0 ledger migration discards potentially poisoned pre-fix boundaries but preserves pending reports, forcing one safe corrected scan. An optional idle watcher can run one additional pass after a reset while the user remains in an eligible screen. Character targeting is the default and safely falls back to class targeting if a successful profile scan returns no usable characters. Mod Options reserves a stable row for every operative slot reported by Darktide (with a ten-slot fallback), binds selections to backend character IDs, and refreshes the roster on Morningstar entry, Operative Selection entry and every five minutes in an eligible context. Renames retain their selection, newly created operatives default on, and deleted IDs require two successful absent discoveries before their rows are reclaimed. Health and Toughness default on, Stamina and Wound default off, the minimum level defaults to 410, and every class defaults on. The buyer waits for automatic discard to settle in Morningstar, re-fetches and exactly revalidates each offer immediately before purchase, selects the wallet belonging to the offer's target character, purchases sequentially, never retries an ambiguous POST, and reports confirmed purchases in one native notification plus an informational log. Curios matching its active acquisition rule are protected from later automatic-discard passes.
...
- Lantern of the Omnissiah compatibility is automatic when its recommendation panel and BetterInventory's inventory-options widget are visible: melee and ranged recommendation windows become the top scrollable panel section, and Lantern's duplicate floating weapon panel is suppressed while hosted. Curio recommendations keep Lantern's standalone placement by default through a separate opt-out setting. The integration also lowers the equipped marker on Character Overview cards to keep it clear of Lantern's recommendation icon.
- ItemSorting compatibility is automatic when that mod is enabled. BetterInventory adds a collapsible `ItemSorting mod` section directly below `Sorting` in its melee, ranged, Curio, Armoury Exchange and GlobalStore panels. Every vanilla-style method is retained under `Darktide Native Sorting`, even when ItemSorting's defaults would hide some of them, while its complete custom set—Family, Family + Mark, and both Base Rating directions—becomes directly selectable in the new section. This does not overwrite ItemSorting's saved Mod Options, and ItemSorting remains the owner of every comparator.
...
- When grid layout is disabled, Hadron exposes a default-on **Mirror single-column format from inventory** switch, while Armoury Exchange exposes a default-on **Enable custom detailed card for single column** switch. Hadron mirrors the inventory card 1:1; Armoury uses a tailored card with extra height and footer clearance. Disabling either switch restores that view's native compact cards; GlobalStore and inventory card geometry are unchanged.
- Character overview now has independent default-on melee and ranged switches that replace the equipped weapon slots with a compact custom detailed card, plus a default-on Curio details switch. An optional default-off **Show only weapon dump stat** mode replaces all five maximum-potential modifier rows on both mirrored weapon cards with the lowest modifier above item power, matching the compact grid treatment. Character Overview-only controls adjust that label's horizontal offset (-10 by default), 50-200% font scale (130% by default), colour preset and custom RGB channels. Detailed overview Curios have an independent 50-150% text-scale control (110% by default) and a Curio title-mode dropdown: no title, an auto-fitted one-line title, or the default auto-fitted title allowed to occupy two lines. Title modes reserve their selected line count plus padding above all four stat rows; long stat descriptions wrap and shift every following row down by their measured height. Empty Curio slots retain Darktide's native empty/locked artwork without an `n/a` label. The overview keeps Darktide's fixed decorative frame, reuses the native quality/mark row for perk content, and renders the lower detail/stat passes above the frame overlay so the cards remain readable without distorting the native layout. The integration is limited to overview widgets; inventory tabs remain unchanged and all dimensions scale with Darktide's UI canvas.
- Exact view gating: Entreat targets only `CraftingMechanicusModifyView`; Hadron's Sacrifice Weapons path and its custom barter grid are unchanged. Requisition and GlobalStore target only their matching `CreditsVendorView` service routes; Brunt's Armoury and `CreditsGoodsVendorView` remain unchanged.
...
- Armoury Exchange cards include a default-on **Brighten Armoury item levels** option so their ratings remain readable above the dark price footer; it affects Armoury cards only.
- An optional, default-on collapsible **Sorting** panel in Requisition Weapons & Curios mirrors the inventory options widget, keeps the equipped/favorite and perfect-roll priorities visible, and nests a collapsible **Darktide Native Sorting** section with every native Armoury sort choice as a direct button. BetterInventory's priorities remain the outer comparator tier, while the selected native power, rarity, price or name order breaks ties within each priority group. The panel option is disabled when the Armoury grid is disabled.
- GlobalStore's Multi-Operative Supply can be enabled or left native through the **Mod Integration: GlobalStore** subsection. Integrated two- or three-column cards keep the weapon art at its original height, place the Ordo Dockets/item-level row above the character portrait/class/name row, and expose **GlobalStore character photo size (%)** (50-125%, default 110%), **class icon size** (default 16 px), **character name font size** (default 16 px), **character info horizontal gap** (default 5 px), and **price/item-level row padding** (default 10 px) controls. The legacy **Compact GlobalStore character names in 4-5 columns** setting remains for profile compatibility but has no effect while the view is capped at three columns. The same collapsible **Sorting** and **Darktide Native Sorting** widget is available when the GlobalStore grid is enabled.
- Native single-column GlobalStore portrait geometry is isolated to the `configuration.global_store` + `native_single_column` path; it never changes the two- or three-column grid cards. The portrait and modifier offsets are logical Darktide UI-canvas coordinates, so the same proportions scale with the native UI across 1920x1080, 2560x1440, 3840x2160 and 5120x1440 layouts. For bottom-aligned portrait passes, **smaller Y offsets move the portrait upward**; keep that sign convention when making future adjustments. Native single-column modifier X/Y placement is exposed only in the GlobalStore subsection.
- The Armoury sorting panel is anchored to Darktide's fit-to-screen UI canvas rather than physical pixels, so its offset and size scale with the native 1920×1080 layout across standard 16:9 resolutions and ultrawide displays.
...
The normal/default configuration does not alter filters or backend transactions. The account-changing discard-management group is disabled by default; enabling it still defaults to Manual with confirmation. Morningstar automation and confirmation skipping each require additional explicit choices. The automatic Curio Buyer remains disabled by default; its Operative Selection trigger and idle refresh rescan are also disabled by default, while the once-per-store-rotation gate is enabled by default. All passes are bounded, sequential, server-time gated, revalidated immediately before writes, and protected by cancellation tokens and compact account-scoped rotation history. While Auto Crafter is active, BetterInventory defers its scheduled automatic discard and Curio Buyer passes. Native or third-party purchase, crafting, mastery, discard, and favorite changes stop Auto Crafter safely between requests; if an Auto Crafter request is unresolved, the competing mutation is rejected with a notification instead of overlapping it. The implementation has been statically checked against Darktide 1.12.3 source, but automatic mode still requires in-game passes for popup cancellation, controller navigation, character switching, backend failures, menu notification visibility, reset timing and every supported resolution before it should be considered stable.

## Supported integrations

The following integrations are supported in v2.2.5. BetterInventory has no optional mod dependencies; each integration activates only when its corresponding mod is installed.
...
- (v1.7.4+) Quick Level Mastery: correct 'Acquire' and 'Sacrifice' button alignment across different resolutions and layouts. Its store, rarity-upgrade, mastery-extraction, and claim service writes share Auto Crafter's mutation guard: a click stops Auto Crafter between requests or is rejected while an Auto Crafter request remains unresolved.
...
- Multi-Operative Supply is controlled separately by the **Mod Integration: GlobalStore** options and uses the installed GlobalStore custom service when enabled.
...
| Brunt's Armoury / Multi-Operative Supply | Unsupported | Brunt unchanged; Multi-Operative Supply implemented through the optional GlobalStore integration |

...
- Character overview now has independent default-on melee and ranged switches that replace the equipped weapon slots with the detailed single-column inventory card, plus a default-on Curio details switch for compact primary/secondary Curio stats. The integration is limited to overview widgets; inventory tabs remain unchanged.

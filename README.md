...
- An experimental quick-discard workflow, disabled by default. When explicitly enabled, the current melee, ranged or Curio inventory gains a compact saved-rarity selector, a Discard button and a context-appropriate protection checkbox below the sorting toggle. The candidate filter supports a maximum item level and per-equipment-type switches, always excludes favorited and equipped gear, protects maximum-total 380-roll weapons by default, and protects Curios at or above a configurable 410 item-level threshold by default. Pressing Discard shows a second Yes/No popup with color-coded counts by rarity. Its Yes callback re-evaluates only the originally previewed gear IDs against the current inventory and current protections before sending the surviving IDs through Darktide's native `event_discard_items` path.
...
The BetterInventory experiment deliberately reuses that final native event/transaction path but does not rely on an earlier preview remaining valid. Its confirmation captures the candidate ID set, then the Yes callback scans the live current-inventory layout again and re-applies rarity, item-level, equipment-type, favorite, equipped, perfect-weapon and high-level-Curio protections. Newly eligible gear cannot enter the confirmed batch because the recheck is intersected with the captured IDs. A changed item can only fall out of the batch.

The perfect-roll guard uses the current five-entry `item.base_stats` model and `Items.total_stats_value`. It protects any five-stat weapon at the maximum 380 total instead of requiring a brittle exact floating-point `80/80/80/80/60` comparison. This may conservatively preserve another maximum-total distribution, which is preferable to a false negative in destructive code.

...
- Two to five responsive columns calculated from the current grid width, with independent **Melee Weapons Columns**, **Ranged Weapons Columns** and **Curios Columns** controls (all default to three). Existing profiles that still have the retired global `columns` value use it as a migration fallback until a per-slot value is saved.
...
- Independent default-on grid switches for Hadron's Entreat Hadron item selector, the Armoury's Requisition Weapons & Curios store, and GlobalStore's Multi-Operative Supply. Hadron and Requisition retain their three-column cap; GlobalStore follows the global two-to-five column setting.
...
The next step is an in-game smoke-test matrix covering the existing character inventories, Entreat Hadron, Requisition Weapons & Curios and GlobalStore Multi-Operative Supply. Verify the Sorting and Darktide Native Sorting sections, controller selection and scrolling, active/owned/unaffordable cards, and GlobalStore character-footer placement at two through five columns. Confirm that disabling Mod Integration: GlobalStore restores its native cards and sorting. Hot enable/disable and coexistence with GlobalStore and active sorting/information mods also need validation.

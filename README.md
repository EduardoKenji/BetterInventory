...
- A Curio-only, default-on “Equipped and favorited items at the top” checkbox below the native sort row, synchronized with the Inventory sorting mod option. Equipped Curios form the first group, favorites the second, and the selected Rating/Rarity/Name comparator continues to order items inside every group. Changing the native sort never disables the priority.
...
The current implementation does not alter filters or backend transactions. Its only sorting change is the optional Curio priority wrapper described above; the native comparators remain authoritative within each priority group. It has been statically checked against Darktide 1.12.3 source, but the new Hadron and Armoury paths plus the on-page Curio checkbox still require mouse/controller, two/three/global-four-or-five-column, purchase-state and hot-enable/disable passes in game.
...
The next step is an in-game smoke-test matrix covering the existing character inventories plus Entreat Hadron and Requisition Weapons & Curios. In the Curio inventory, verify that the inline priority checkbox is visible below the native sort row, is clickable at supported resolutions, mirrors the mod option, places equipped then favorite Curios first under every native sort, and reorders immediately after changing a favorite. On a weapon whose stored stat is fractional, confirm that precision changes only the normal attribute numerator and that an expertise preview remains native. For both vendor views, test two and three global columns, verify that global four/five still renders exactly three, exercise mouse/controller selection and scrolling, and check active/owned/unaffordable Armoury cards. Confirm that Sacrifice Weapons, Brunt's Armoury and Multi-Operative Supply remain native. Hot enable/disable and coexistence with GlobalStore and active sorting/information mods also need validation.

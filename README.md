...
- When grid layout is disabled, the Hadron and Armoury Exchange subsections expose separate default-on **Mirror single-column format from inventory** switches. Each reuses the detailed inventory single-column card geometry and content for its own view; disabling either switch restores that view's native compact cards.

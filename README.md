...
The normal/default configuration does not alter filters or backend transactions. The account-changing discard-management group is disabled by default; enabling it still defaults to Manual with confirmation. Morningstar automation and confirmation skipping each require additional explicit choices. The automatic Curio Buyer remains disabled by default; its Operative Selection trigger and idle refresh rescan are also disabled by default, while the once-per-store-rotation gate is enabled by default. All passes are bounded, sequential, server-time gated, revalidated immediately before writes, and protected by cancellation tokens and compact account-scoped rotation history. The implementation has been statically checked against Darktide 1.12.3 source, but automatic mode still requires in-game passes for popup cancellation, controller navigation, character switching, backend failures, menu notification visibility, reset timing and every supported resolution before it should be considered stable.

...
- Compact Armoury store cards preserve both item power and purchase state. A dedicated 34 px footer keeps wallet/price or owned text in the lower-left and item power in the lower-right. Blessings are anchored above that footer, and at the default font sizes automatic card height grows to 114 px for those rows, 126 px for a detailed four-line Curio, or 148 px when two weapon-perk rows are also enabled.
...
- Store grids use one blueprint height per category/row, so card height cannot safely vary item-by-item. BetterInventory instead computes a uniform height from the enabled content profile, the separate primary/secondary Curio font sizes and the store footer: this guarantees that the largest enabled card (including a Curio innate stat plus three perks) fits without destabilizing grid navigation.

...
- Optional two-line weapon perk text, disabled by default to preserve the compact profile. No Compression keeps Darktide's full localized descriptions; Compression uses labels such as `+25% Flak Damage`, `+25% Maniacs Damage`, `+5% Melee Crit Chance` and `+10% Specialist Melee Dmg`; Heavy Compression shortens these further for narrow cards. Unknown perk identifiers retain the native description.
...
- Weapon perks are the entries in `item.perks`. BetterInventory resolves the master item and classifies its stable `trait` identifier before choosing a compact label, so the mapping does not depend on matching the rendered English sentence. Melee and ranged perks remain distinct where their wording differs.

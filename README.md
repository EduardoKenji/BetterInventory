...
- Default-on Armoury window expansion with a 190–230 px target-card-width slider. The 230 px default widens Darktide's three-column grid from 596 to 710 px, while two-column cards remain at their already wider native size. The grid frame grows with it; stats, comparison panels and the Acquire button shift only through the canvas's safe 64 px margin so comparison remains on screen.
...
- The optional 230 px target produces a 710 px three-column grid. The item-grid pivot and dividers take the complete 114 px expansion. Darktide's two 530 px stats panels otherwise finish only 64 px inside the 1920 px canvas, so their pivots and the Acquire button shift by at most 64 px; the remaining width comes from the native gap between the grid and details panel.

...
- When grid layout is disabled, the Hadron and Armoury Exchange subsections expose separate default-on **Enable custom detailed card for single column** switches. Each uses BetterInventory's custom detailed card geometry and content for its own view; disabling either switch restores that view's native compact cards. Armoury cards add a small amount of height and keep their modifier rows clear of the native price footer; GlobalStore and inventory card geometry are unchanged.

...
- GlobalStore's Multi-Operative Supply can be enabled or left native through the **Mod Integration: GlobalStore** subsection. Integrated three-to-five-column cards keep the weapon art at its original height, place the Ordo Dockets/item-level row above the character portrait/class/name row, and expose **GlobalStore character photo size (%)** (50-125%, default 110%), **class icon size** (default 16 px), **character name font size** (default 16 px), **character info horizontal gap** (default 5 px), and **price/item-level row padding** (default 10 px) controls. The same collapsible **Sorting** and **Darktide Native Sorting** widget is available when the GlobalStore grid is enabled.

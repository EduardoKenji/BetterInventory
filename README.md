...
- Name It compatibility: a default-on integration lets custom weapon and Curio names take priority over BetterInventory's mark-appending format. Its default-on detailed-Curio option reserves a responsive two-line name area above the innate attribute and all three perks in Character Overview, Inventory, Hadron, the Armoury Exchange, GlobalStore and other supported grids. Disabling the integration restores BetterInventory names such as `Dueling Sword Mk IV` and disables every dependent Name It option.

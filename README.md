...
- A default-off scalable inventory-options panel prototype for in-game research. After reopening an inventory, the prototype migrates the same synchronized Sorting and discard controls into one Darktide-managed bounded panel with masked scrolling and clickable collapsible section headers. Its height shrinks with collapsed content and caps at 320 reference pixels so additional rows scroll instead of extending into the footer. Native discard mode presents Sorting only. The established loose-widget implementation remains the automatic fallback if the panel cannot initialize or the prototype setting is disabled.
- An experimental discard-management workflow, disabled by default and set to **Manual** when enabled. Every melee, ranged and Curio inventory gains compact synchronized controls for Manual/Automatic mode, rarity threshold, maximum item level, included equipment types, perfect-weapon protection, high-level-Curio protection and its item-level threshold. The Automatic-only **Skip confirmation prompts** setting is disabled by default; the manual inventory button always retains its confirmation. Automatic mode schedules one pass per Morningstar entry, waits five seconds, requires Darktide's authoritative hub mode and selected character, and fetches the character inventory through `GearService`. It either shows the same color-coded preview or, when explicitly configured to skip it, proceeds without a popup; confirmation-enabled scans also report when no eligible items were found. Both automatic paths fetch the inventory again and intersect the revalidated candidates with the originally captured gear IDs immediately before deletion. All paths always exclude favorited and equipped gear, protect completed and underpowered weapons whose maximum-expertise distribution is four displayed 80s plus a fifth stat of at least 60 by default, and protect Curios at or above the configurable 410 threshold by default. BetterInventory hides all discard-management controls while Darktide's native discard panel is active; only Sorting remains. The compact Mode and conditional confirmation checkbox share one row, so Curios do not need a resolution-sensitive Equip-button move.

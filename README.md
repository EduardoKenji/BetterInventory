...
- Weapon perk and blessing-text rows collapse embedded whitespace to a single line before fitting. Perks keep a more legible 9 px minimum and a drop shadow over weapon art; blessing names can shrink to 8 px. Both use an ellipsis only when still too wide. Native perk-rank symbols have an independent 12–32 px size control (17 px by default). Automatic height uses the selected blessing mode, both configured icon sizes and any store footer when reserving content regions, instead of allowing them to collide with one another or item power.

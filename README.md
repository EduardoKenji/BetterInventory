...
- A grid-layout switch: disabling it preserves Darktide's native single-column width, layout behavior and icon lifecycle while retaining enabled BetterInventory content enhancements. Single-column cards reserve additional height for BetterInventory's perk and blessing rows. When Quick Look Card is installed, BetterInventory retains and compacts only its five weapon modifier stats; Quick Look Card's base rating, perk and blessing fields are hidden to avoid duplicate content. All Quick Look Card passes remain intentionally hidden in multi-column mode.

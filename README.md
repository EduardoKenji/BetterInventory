...
- Default-on weapon blessing symbols using Darktide's current composited blessing material; the native ranked frame visibly carries each blessing level. Icon size is configurable from 20–48 px (34 px by default), with a configurable 0–20 px horizontal gap defaulting to 3 px.
...
- Weapon perk rows are forced to one line, shrink to an 8 px minimum and then use an ellipsis. Their native rank symbols have an independent 12–32 px size control (18 px by default). Automatic height uses both configured icon sizes when reserving the perk, blessing and store-footer regions, instead of allowing them to collide with one another or item power.
...
- A default All four stats Curio profile showing the innate stat and all three perks, plus a compact Primary stat profile that retains the Curio name and power while adding only its innate line.

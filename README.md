...
- An experimental quick-discard workflow, disabled by default. When explicitly enabled, every melee, ranged and Curio inventory gains compact controls for the saved rarity threshold, maximum item level, included equipment types, perfect-weapon protection, high-level-Curio protection and its item-level threshold. These are alternate editors for the same persistent mod-option values. The candidate filter uses the character inventory cached by the parent inventory view, always excludes favorited and equipped gear, protects true perfect 380 rolls by default, and protects Curios at or above the configurable 410 threshold by default. Pressing Discard shows a second Yes/No popup with color-coded counts by rarity and an optional default-on melee/ranged/Curio breakdown. Its Yes callback re-evaluates only the originally previewed gear IDs against the current inventory cache and current protections before sending the surviving IDs through Darktide's native `event_discard_items` path.
...
The perfect-roll guard uses the current five-entry `item.base_stats` model and `Items.total_stats_value`. It requires total base power 380, exactly four individually rounded visible attributes at 80 and one remaining visible attribute at 60 or above. Darktide totals the unrounded fractional values before rounding the total, while the UI rounds each attribute independently; this correctly retains legitimate visible `80/80/80/80/61` and `80/80/80/80/62` anomalies without treating every 380 distribution as perfect.

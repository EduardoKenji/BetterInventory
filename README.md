...
- A four-mode weapon blessing dropdown: ranked **Icons** by default, compact **Text lines**, **Tier symbols + text**, or **Off**. Icon mode uses Darktide's current composited blessing material and ranked frame; icon size is configurable from 20–48 px (36 px by default), with a configurable 0–20 px horizontal gap defaulting to 3 px. Text mode prefixes each localized name with Darktide's authoritative Roman rank, for example `III Surgical`; Tier-symbol mode pairs the same name with Darktide's standalone native I–IV perk-rank material. Blessing text defaults to a light green (`RGB 190, 210, 180`) and has synchronized presets and RGB controls. By default, four- and five-column grids put both text rows above a dedicated item-level row, giving each name the remaining card width; the threshold can be set to Always, five columns only, or Never.
...
- Weapon perk and blessing-text rows collapse embedded whitespace to a single line before fitting. Perks keep a more legible 9 px minimum and a drop shadow over weapon art; blessing names can shrink to 8 px. Both use an ellipsis only when still too wide. Native rank symbols shared by perks and Tier-symbol blessing text have an independent 12–32 px size control (17 px by default). Perk rows and blessing-text rows each have independent 0–20 px vertical-gap controls (both default to 1 px), plus a 0–20 px control for padding between the two sections (3 px by default) and a 0–20 px bottom-padding control below the second blessing row (1 px by default). Automatic height uses the selected blessing mode, configured icon sizes, all four vertical-spacing settings and any store footer when reserving content regions, instead of allowing them to collide with one another or item power.

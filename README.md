...
- A final **Equipped item highlight** subsection at the bottom of **Card content**, with a selector offering **Off**, **Soft glow**, **Animated dashed border**, and **Solid border**. The default is a gold Animated dashed border at width 3. Soft glow has an independent 0-100% intensity slider; the two border modes have independent 1-5 width sliders. Soft glow's mode default is white, while both borders use the Auto Crafter terminal gold. Shared presets and RGB sliders apply to every mode, and editing a channel selects Custom colour. Only the selected mode's intensity/width control is active. The animated mode uses Darktide's native GPU-animated dashed-frame material, requiring no Lua timer or retained per-card animation state; thicker borders use at most five bounded native passes which are replaced during blueprint recomposition. Every mode follows Darktide's native equipped state without hiding its equipped symbol.
- A separate final **Newly acquired item highlight** subsection that replaces Darktide's small new-item dot with a gold Animated dashed border by default. Native dot only, Soft glow and Solid border modes are also available, with the same bounded width/intensity and preset/RGB controls. BetterInventory follows Darktide's authoritative persisted new-item marker rather than maintaining acquisition history. Items are acknowledged through Darktide's native removal callback when selected by default; an optional Hover mode acknowledges them as soon as a mouse pointer enters the card, while controller focus also counts as acknowledgement. Only one pass evaluates acknowledgement, callback failures are contained, and enhanced layers are bounded and replaced during blueprint recomposition.

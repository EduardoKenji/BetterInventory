...
- A final **Equipped item highlight** subsection at the bottom of **Card content**, with a default-on selector offering **Off**, **Soft glow**, **Animated dashed border**, and **Solid border**. Soft glow has an independent 0-100% intensity slider; the two border modes have independent 1-5 width sliders. Soft glow defaults to white, while both borders default to the Auto Crafter terminal gold. Shared presets and RGB sliders apply to every mode, and editing a channel selects Custom colour. Only the selected mode's intensity/width control is active. The animated mode uses Darktide's native GPU-animated dashed-frame material, requiring no Lua timer or retained per-card animation state; thicker borders use at most five bounded native passes which are replaced during blueprint recomposition. Every mode follows Darktide's native equipped state without hiding its equipped symbol.

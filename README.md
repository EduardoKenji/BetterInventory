...
- An **Automatic Curio Buyer**, disabled by default, with synchronized controls in mod options and the scalable inventory panel. It scans the normal Armoury Exchange once per Morningstar entry across all saved characters and can filter by displayed item level, Health, Toughness, Stamina, Wound, class or individual character. Character targeting is the default and safely falls back to class targeting if a successful profile scan returns no usable characters. Mod Options reserves a stable row for every operative slot reported by Darktide (with a ten-slot fallback), binds selections to backend character IDs, and refreshes the roster on Morningstar entry and every five minutes in the hub. Renames retain their selection, newly created operatives default on, and deleted IDs require two successful absent discoveries before their rows are reclaimed. Health and Toughness default on, Stamina and Wound default off, the minimum level defaults to 410, and every class defaults on. The buyer waits for automatic discard to settle, re-fetches and exactly revalidates each offer immediately before purchase, selects the wallet belonging to the offer's target character, purchases sequentially, never retries an ambiguous POST, and reports confirmed purchases in one native notification. Curios matching its active acquisition rule are protected from later automatic-discard passes.

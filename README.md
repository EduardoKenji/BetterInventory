...
- A default-off scalable inventory-options panel prototype for in-game research. After reopening an inventory, the prototype migrates the same synchronized Sorting and discard controls into one Darktide-managed bounded panel with masked scrolling and clickable collapsible section headers. The panel exposes width, maximum height, row spacing and independent top/bottom/left/right padding controls; its 445 x 360 px defaults shrink with collapsed content, while additional rows scroll instead of extending into the footer. Curio views additionally expose full information-panel width and upper preview-area height percentages. Preview art scales on both axes with the upper area, preserving its aspect ratio. Native discard mode presents Sorting only. The established loose-widget implementation remains the automatic fallback if the panel cannot initialize or the prototype setting is disabled.

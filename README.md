...
- An experimental discard-management workflow, disabled by default and set to **Manual** when enabled. Every melee, ranged and Curio inventory gains compact synchronized controls for Manual/Automatic mode, rarity threshold, maximum item level, included equipment types, perfect-weapon protection, high-level-Curio protection and its item-level threshold. The Automatic-only **Skip confirmation prompts** setting is disabled by default; the manual inventory button always retains its confirmation. Automatic mode schedules one pass per Morningstar entry, waits five seconds, requires Darktide's authoritative `hub` game mode and selected character, and fetches the character inventory through `GearService`. It either shows the same color-coded preview or, when explicitly configured to skip it, proceeds without a popup. Both automatic paths fetch the inventory again and intersect the revalidated candidates with the originally captured gear IDs immediately before deletion. All paths always exclude favorited and equipped gear, protect completed and underpowered weapons whose maximum-expertise distribution is four displayed 80s plus a fifth stat of at least 60 by default, and protect Curios at or above the configurable 410 threshold by default. BetterInventory hides all discard-management controls while Darktide's native discard panel is active; only Sorting remains. The compact Mode and conditional confirmation checkbox share one row, so Curios do not need a resolution-sensitive Equip-button move.

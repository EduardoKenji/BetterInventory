...
- A default-on “Equipped and favorited items at the top” checkbox in every melee, ranged and Curio inventory, synchronized through one persistent Inventory sorting mod option. The Curio control follows the bottom of the dynamically sized item-details panel; weapon controls sit below Marks/Cosmetics/Inspect. Equipped items form the first group, favorites the second, and the selected Rating/Rarity/Name comparator continues to order items inside every group. Changing the native sort never disables the priority. While Darktide's native discard panel is active, the checkbox updates the saved preference without re-presenting its temporary filtered grid; Darktide applies the current comparator once when ESC restores the full inventory.
- An experimental quick-discard workflow, disabled by default. When explicitly enabled, every melee, ranged and Curio inventory gains compact controls for the saved rarity threshold, maximum item level, included equipment types, perfect-weapon protection, high-level-Curio protection and its item-level threshold. These are alternate editors for the same persistent mod-option values. The candidate filter uses the character inventory cached by the parent inventory view, always excludes favorited and equipped gear, protects completed and underpowered weapons whose maximum-expertise distribution is four displayed 80s plus a fifth stat of at least 60 by default, and protects Curios at or above the configurable 410 threshold by default. Pressing Discard shows a second Yes/No popup with color-coded counts by rarity and an optional default-on melee/ranged/Curio breakdown. Its Yes callback re-evaluates only the originally previewed gear IDs against the current inventory cache and current protections before sending the surviving IDs through Darktide's native `event_discard_items` path. BetterInventory hides all of its quick-discard controls while Darktide's native discard panel is active; only the Sorting group remains, positioned beneath the dynamically sized item-information panel and to the left of Darktide's native panel. Curio quick-discard controls use the same compact width as weapon controls even though the Curio information panel is wider.
...
The normal/default configuration does not alter filters or backend transactions. The only account-changing feature is the separately grouped experimental quick-discard workflow, which is opt-in, bounded by explicit equipment-type/rating/rarity protections and confirmation-gated. No login-triggered prompt, unattended deletion or automatic Morningstar operation is implemented. The native comparators remain authoritative within each sorting-priority group. The implementation has been statically checked against Darktide 1.12.3 source, but the new Hadron and Armoury paths, on-page inventory controls and especially quick discard still require mouse/controller, resolution, cancellation, empty-result, protected-item and real backend passes in game.
...
Only bounded, explicit-confirmation experiments may enter this phase after their native API path and failure modes are audited. Unattended execution remains excluded.
...
The BetterInventory experiment deliberately reuses that final native event/transaction path but does not rely on an earlier preview remaining valid. Its confirmation captures the candidate ID set from the character inventory cached by `InventoryBackgroundView`, then the Yes callback scans that live cache again and re-applies rarity, item-level, equipment-type, favorite, equipped, perfect-weapon and high-level-Curio protections. Newly eligible gear cannot enter the confirmed batch because the recheck is intersected with the captured IDs. A changed item can only fall out of the batch.
...
The first experiment intentionally stops at a visible manual action and a second confirmation. The proposed Morningstar-login scan, automatically shown cleanup prompt and unattended auto-discard mode are not implemented yet. Before any of them is considered, the manual path needs in-game validation for popup cancellation, controller navigation, favorites changed while the popup is open, equipment changes, backend failures, batches over 40 items and every supported resolution. A future preview prompt should reuse this exact candidate engine and remain non-destructive until the user confirms its captured list.

...
- An **Automatic Curio Buyer**, disabled by default, with synchronized controls in mod options and the scalable inventory panel. It scans the normal Armoury Exchange once per Morningstar entry across all seven saved-character archetypes and can filter by displayed item level, Health, Toughness, Stamina, Wound and class. Health and Toughness default on, Stamina and Wound default off, the minimum level defaults to 410, and every class defaults on. The buyer waits for automatic discard to settle, re-fetches and exactly revalidates each offer immediately before purchase, selects the wallet belonging to the offer's target character, purchases sequentially, never retries an ambiguous POST, and reports confirmed purchases in one native notification. Curios matching its active acquisition rule are protected from later automatic-discard passes.
